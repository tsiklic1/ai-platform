...
- **Physical scale must stay consistent.** State each product's real-world physical size (height in cm, volume in ml/oz/g, or a clear relative descriptor like "palm-sized") and keep at least one co-visible scale-reference object in every frame — a hand, glass, plate, basket, phone, or person's torso. The product's size relative to that scale reference must NOT drift between frames. The image model has no innate sense of how big things are; without a scale anchor it will guess differently each frame and you'll get a juice can the size of a bicycle.
- **Product list is authoritative.** Only feature products that actually exist for the brand. Do not invent new variants, flavors, sizes, colors, or sub-brands that aren't part of the brand's product catalog.
- **Frame 1 must contain the product visible and at correct scale.** The model uses frame 1 as its visual anchor for everything that follows. Never start with the product invisible / off-screen / a tiny silhouette in the distance — middle frames have no reference to inherit from and will hallucinate the product's size.
...
- **Dramatic camera push-ins (far → extreme close-up)** — every frame re-frames the product, so scale has to be re-decided every step. If you must do a push-in, keep the product visible AND at correct scale in frame 1, then move the camera 10-20% closer each frame. Never start from "subject barely visible in the distance" — the model has no anchor to inherit and middle-frame product proportions will explode.
